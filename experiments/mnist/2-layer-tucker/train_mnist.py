...
def main(_):
    ...
    err_rslt = []
    compres_rate = []
    for rank_val in range(4,12): #8 points
        extra_opts={}
        ...
        rate = compres_ratio_tucker(layers, tucker_layer_ranks)
        compres_rate.append(rate)
        rslt = run_training(extra_opts)
        err_rslt.append(1.0-rslt['precision'])
    with open('./results/res_tucker_LeNet300','w') as f:
        for i in range(len(err_rslt)):
            f.write('{0:.4f}:{0:.5f}\t'.format(compres_rate[i],err_rslt[i]))
