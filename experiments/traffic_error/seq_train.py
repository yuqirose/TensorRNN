...
flags.DEFINE_string("save_path", "/tmp/tensorcompress/log/traffic_error_exp/basic_rnn/",
...
def run_epoch_eval(session, model, eval_config, verbose=False):
    """Runs the model on the evaluation data, back feeding the prediction"""
    start_time = time.time()
    costs = 0.0
    predicts = np.array([]).reshape(0,model.input.vocab_size)
    targets = []
    ...
    for step in range(model.input.epoch_size):
        ...
        with tf.name_scope("Test_Model_S"+str(step)):
            ...
            test_data = np.concatenate((inputs[0,:-1,:],predict), axis=0)
