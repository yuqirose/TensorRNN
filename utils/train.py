...
def create_commands(session, num_workers, remotes, env_id, logdir, shell='bash', mode='tmux'):
  ...
  if mode == 'tmux':
    cmds_map += [new_cmd(session, "top", ["top"], mode, logdir, shell)]
...
def maybe_create_dir(p):
  print("Maybe creating %s", p)
...
def run():
  ...
  print("Purging top-level logdir %s. If checkpoint exists, Supervisor "
